	...
	<view class="uni-steps">
		<view class="uni-steps-items" :class="'uni-steps-' + direction">
			<view class="uni-steps-item" v-for="(item,index) in steps" :key="index" :class="[item.status ? 'uni-steps-' + item.status : '']">
				<view class="uni-steps-item-title-container" :style="{color:''}">
					<view class="uni-steps-item-title">{{ item.title }}</view>
					<view class="uni-steps-item-desc">{{ item.desc}}</view>
				</view>
				<view class="uni-steps-item-circle-container">
					<view class="uni-steps-item-circle" v-if="item.status !== 'process'" :style="{backgroundColor:activeColor}"></view>
					<!-- <uni-icon v-else type="checkbox-filled" size="14" :color="activeColor"></uni-icon> -->
				</view>
				<view class="uni-steps-item-line" v-if="index !== steps.length-1" :style="{backgroundColor:activeColor}"></view>
			</view>
		</view>
	</view>
</template>
...
<style lang="scss">
	$steps-line-color:#5871FE;
	$steps-text-color:#282828;
	$steps-title-size:28upx;
	$steps-desc-size:24upx;
	$steps-text-finish-color:#282828;
